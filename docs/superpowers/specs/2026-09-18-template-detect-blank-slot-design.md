...
- **已知窄缝**：识别落库后 B 端手动增删占位符不经 `_verify_slot_occ`——同段恰有「严格更长且区间相交的同形锚」时 verify 的 blocked 集与渲染侧可偏差（三重巧合，低概率；渲染 no-op 安全侧留 warning，反向错位概率更低）。记录为已知局限，不在本次修
- 纯空白下划线段落（整行下划线独立填写行）已实测入候选（真实范本 57 行，commit a0385f8b 根修 strip 早退）
