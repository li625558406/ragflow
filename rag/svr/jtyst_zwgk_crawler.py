...
#
"""
Crawler for jtyst.fujian.gov.cn — 福建省交通运输厅 政务公开 (today's data).

Covers three URL targets:
  1. /zwgk/          — main page with multiple tabbed sections
  2. /zwgk/tzgg/     — 公告通告 sub-page
  3. /jdhy/zcjd/     — 政策解读 sub-page (政策图解/文字解读/上级解读)
...
Usage
-----
    python jtyst_zwgk_crawler.py \
        --tenant-id <TENANT_ID> \
        --kb-id <KB_ID> \
        --task-name <NAME>

    # Optional:
        --section zwgk        # Only main page sections
        --section tzgg        # Only 公告通告
        --section zcjd        # Only 政策解读
...
def _extract_date_from_url(url):
    ...
    """
    # Full date from filename: t20260522_
...
def _parse_zwgk_main(html_bytes, base_url, target_date):
    """Parse the /zwgk/ main page for all section items matching target_date.

    The main page has multiple content blocks:
      - .zwgk_left: tabbed sections (政策法规/财政资金, 发展规划/统计信息, etc.)
      - .zwgk_right: sidebar (公告通告, 政策解读)

    Dates are in MM-DD format in <span> elements. URLs contain the full date.
    ...
    def _matches_target(date_str_from_url, span_text):
        """Check if an article matches the target date."""
        if span_text and re.match(r'^\d{2}-\d{2}$', span_text):
            return span_text == target_mmdd
        if date_str_from_url:
            return date_str_from_url[5:] == target_mmdd
        return False

    def _resolve_date(span_text, url_date):
        """Get full date string."""
        if span_text and re.match(r'^\d{2}-\d{2}$', span_text):
            return f"{target_date[:5]}{span_text}"
        return url_date or ""

    # ── Main content area (.zwgk_left) ──────────────────────────────────
    left_area = soup.select_one(".zwgk_left")
    if left_area:
        for ul in left_area.find_all("ul", class_="list"):
            for li in ul.find_all("li"):
                ...
                # Skip external links
                if ("gov.cn" in href and "jtyst" not in href) or href.startswith("javascript:") or href == "#":
                    continue

                span_text = span.get_text(strip=True) if span else ""
                url_date = _extract_date_from_url(href)

                if not _matches_target(url_date, span_text):
                    continue
                ...
                articles.append({
                    ...
                    "date_str": _resolve_date(span_text, url_date),
                    "article_id": article_id,
                    "section": "政务公开-主页面",
                })

        # Also check carousel/news area (.zwgk_news)
        for container in left_area.find_all("div", class_="zwgk_news"):
    ...
    # ── Sidebar (.zwgk_right) ───────────────────────────────────────────
    right_area = soup.select_one(".zwgk_right")
    if right_area:
        for container in right_area.find_all("div", class_="zfxxgk_right_box"):
            # Get section name from heading
            h4 = container.find("h4")
            section_name = h4.get_text(strip=True) if h4 else "政务公开"
            ...
            for ul in container.find_all("ul"):
                for li in ul.find_all("li"):
                    ...
                    articles.append({
                        ...
                        "date_str": url_date,
                        "article_id": article_id,
                        "section": f"政务公开-{section_name}",
                    })

    return articles


def _parse_tzgg_listing(html_bytes, base_url, target_date):
    """Parse the /zwgk/tzgg/ page for items matching target_date.

    All articles are server-rendered in the HTML. Avalon.js pagination
    groups ($showStatic(N)) are all present in the initial HTML.
    """
    if not html_bytes:
        return []
...
def _parse_zcjd_listing(html_bytes, base_url, target_date):
    """Parse the /jdhy/zcjd/ page for items matching target_date.

    Three sub-sections: 政策图解 (zctj/), 文字解读 (wzjd/), 上级解读 (sjjd/).
    Each has a .gl_tit1 header and .jtt-gl_list with <ul>.
    """
    if not html_bytes:
        return []
...
def _parse_detail_html(html_bytes, detail_url):
    ...
    # Attachments: .myzj_xl_list links (both visible and ms-visible hidden)
        ...
        if download_dir and os.path.isdir(download_dir):
            ...
            for att in attachments:
                ...
                if not os.path.exists(local_path) and os.path.exists(alt_path):
                    local_path = alt_path
                # Also check extracted subdirectories
...
def parse_args():
    p = argparse.ArgumentParser(
        description="jtyst.fujian.gov.cn zwgk crawler — 福建省交通运输厅 政务公开 (today only)"
    )
    p.add_argument("--tenant-id", required=True)
    p.add_argument("--kb-id", required=True)
    p.add_argument("--task-name", required=True)
    p.add_argument("--target-url", default=None,
                   help="Target URL — auto-detects section (e.g. https://jtyst.fujian.gov.cn/zwgk/tzgg/)")
    p.add_argument("--output-dir", default=None,
                   help="Output root directory (default: PROJECT_ROOT/rag/<task_name>)")
    p.add_argument("--section", default="all",
                   choices=["all", "zwgk", "tzgg", "zcjd"],
                   help="Section to crawl (default: all, overridden by --target-url if given)")
...
def _detect_section_from_url(url):
    """Detect which section a URL belongs to. Returns section key or None."""
    if not url:
        return None
    url_lower = url.lower()
    if "/zwgk/tzgg" in url_lower:
        return "tzgg"
    if "/jdhy/zcjd" in url_lower:
        return "zcjd"
    if "/zwgk" in url_lower:
        return "zwgk"
    return None
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[JTYST-ZWGK] 福建省交通运输厅 — 政务公开 crawler")
    _safe_print("[JTYST-ZWGK] Section: {}".format(args.section))
    _safe_print("[JTYST-ZWGK] KB: {}".format(args.kb_id))
    _safe_print("[JTYST-ZWGK] Task: {}".format(args.task_name))
    _safe_print("[JTYST-ZWGK] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== JTYST-ZWGK crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[JTYST-ZWGK] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[JTYST-ZWGK] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    target_date = _get_target_date()
    _safe_print("[JTYST-ZWGK] Target date: {} (8AM rule)".format(target_date))
    sys.stdout.flush()
    ...
    if args.target_url:
        ...
        if detected:
            ...
            _safe_print("[JTYST-ZWGK] Section auto-detected from URL: {}".format(detected))
            sys.stdout.flush()
        else:
            sections_to_crawl = ["zwgk", "tzgg", "zcjd"] if args.section == "all" else [args.section]
    else:
        sections_to_crawl = ["zwgk", "tzgg", "zcjd"] if args.section == "all" else [args.section]
    sections_to_crawl = [s for s in sections_to_crawl if s not in completed_sections]

    if not sections_to_crawl:
        _safe_print("[JTYST-ZWGK] All sections already completed, nothing to do.")
        sys.stdout.flush()
        return

    # ── Step 1: Crawl listing pages ────────────────────────────────────
    _safe_print("[JTYST-ZWGK] Step 1/3: Crawling listing pages...")
    sys.stdout.flush()

    all_articles = []
    for section_name in sections_to_crawl:
        listing_url = _SITE_ROOT + _LISTING_URLS[section_name]
        _safe_print("[JTYST-ZWGK]   Fetching {}: {}".format(section_name, listing_url))
        sys.stdout.flush()

        html_bytes = _http_get(listing_url, referer=_SITE_ROOT + "/")
        if not html_bytes:
            _safe_print("[JTYST-ZWGK]   Failed to fetch {}, skipping.".format(section_name))
            sys.stdout.flush()
            continue

        if section_name == "zwgk":
            articles = _parse_zwgk_main(html_bytes, listing_url, target_date)
        elif section_name == "tzgg":
            articles = _parse_tzgg_listing(html_bytes, listing_url, target_date)
        elif section_name == "zcjd":
            articles = _parse_zcjd_listing(html_bytes, listing_url, target_date)
        else:
            articles = []

        _safe_print("[JTYST-ZWGK]   -> {} article(s) for {}".format(len(articles), target_date))
        sys.stdout.flush()
        all_articles.extend(articles)

        time.sleep(random.uniform(_REQUEST_DELAY_MIN, _REQUEST_DELAY_MAX))

    _safe_print("[JTYST-ZWGK] Total articles for {}: {}".format(target_date, len(all_articles)))
    sys.stdout.flush()

    if not all_articles:
        _safe_print("[JTYST-ZWGK] No articles found for today, marking sections complete.")
        sys.stdout.flush()
    ...
    if skipped:
        _safe_print("[JTYST-ZWGK] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[JTYST-ZWGK] All articles for today already processed.")
        sys.stdout.flush()
        for s in sections_to_crawl:
            completed_sections.add(s)
        state["completed_sections"] = list(completed_sections)
    ...
    _safe_print("\n[JTYST-ZWGK] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        if elapsed > args.max_runtime - 120:
            _safe_print(
                "\n[JTYST-ZWGK] Runtime {:.0f}s approaching limit ({}s), "
        ...
        _safe_print("[JTYST-ZWGK] [{}/{}] [{}] {}...".format(
            idx, len(new_articles), section_label, title[:50]))
        sys.stdout.flush()

        # Fetch detail page
        html_bytes = _http_get(detail_url, referer=_SITE_ROOT + "/zwgk/")
        if not html_bytes:
        ...
        folder_name = _sanitize_filename(
            "{}_{}_{}".format(date_str, article_id[:12], title[:40]), max_len=120)
        md_path = os.path.join(output_dir, f"{folder_name}.md")
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[JTYST-ZWGK]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                _safe_print("[JTYST-ZWGK]   Uploaded to KB {}".format(args.kb_id))
                sys.stdout.flush()
            except Exception as e:
                logging.error("KB upload failed: %s", e)
                _safe_print("[JTYST-ZWGK]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            ...
            _safe_print("[JTYST-ZWGK]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[JTYST-ZWGK] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[JTYST-ZWGK] Stopped early, will resume next run")
    _safe_print("[JTYST-ZWGK] Target date: {}".format(target_date))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== JTYST-ZWGK crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "jtyst_zwgk_crawler"
