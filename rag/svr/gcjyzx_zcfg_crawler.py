...
#
"""
Crawler for gcjyzx.zhangzhou.gov.cn — 政策法规 (zcfg) section.

Static server-rendered listing with JavaScript pagination.  ~439 articles
across 22 pages.

Data flow
─────────
  1. **Listing**: GET /zcfg/moreinfo.html (page 1), /zcfg/{N}.html (page N)
     → Parse <li class="list-item"> entries for title, URL, date.
  2. **Detail**: GET /zcfg/{YYYYMMDD}/{uuid}.html  (SSR HTML)
     → Parse h3.title, .toolbar metadata, .content-main body, attachment links.
  3. **Attachments**:
     - /zzggzy/{infoid}/{fileuuid}/{filename} → direct binary download
     - ztbfjyz(...) onclick → same path extraction as jyxx crawler
...
Checkpoint/resume: state saved every 10 articles.  Time-bounded check
(default 3300s) stops gracefully before the 3600s task-timeout window.

Usage
-----
    python gcjyzx_zcfg_crawler.py \
        --tenant-id <TENANT_ID> \
        --kb-id <KB_ID> \
        --task-name <NAME>
...
_LISTING_PATH = "/zcfg/moreinfo.html"
_SECTION_LABEL = "政策法规"

_PAGE_SIZE = 20  # items per listing page
...
def _extract_infoid_from_url(url):
    """Extract article UUID from detail URL like /zcfg/20260212/{uuid}.html."""
...
def _crawl_listing_page(page_num):
    ...
    if page_num <= 1:
        ...
        url = f"{_SITE_ROOT}/zcfg/{page_num}.html"
    ...
    # Last page: fewer items than _PAGE_SIZE
    has_more = len(articles) >= _PAGE_SIZE
...
def _parse_detail_html(html_bytes, detail_url):
    """Parse a zcfg article detail page.
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        # Type d: document extensions (pdf/doc/xlsx) in href
...
def parse_args():
    p = argparse.ArgumentParser(
        description="gcjyzx.zhangzhou.gov.cn zcfg crawler — 漳州市工程项目交易中心 政策法规"
    )
    p.add_argument("--tenant-id", required=True)
    p.add_argument("--target-url",
                   default="https://gcjyzx.zhangzhou.gov.cn/zcfg/moreinfo.html")
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZCFG] 漳州市工程项目交易中心 — {} crawler".format(_SECTION_LABEL))
    _safe_print("[ZCFG] Target: {}".format(args.target_url))
    _safe_print("[ZCFG] KB: {}".format(args.kb_id))
    _safe_print("[ZCFG] Task: {}".format(args.task_name))
    _safe_print("[ZCFG] Max articles: {}".format(
        args.max_articles if args.max_articles else "unlimited"))
    _safe_print("[ZCFG] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== ZCFG crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[ZCFG] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    if state.get("completed"):
        _safe_print("[ZCFG] Already completed, nothing to do.")
        sys.stdout.flush()
        return

    _safe_print("[ZCFG] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    crawl_start = time.time()

    # ── Step 1: Crawl all listing pages ────────────────────────────────
    _safe_print("[ZCFG] Step 1/3: Crawling listing pages...")
    sys.stdout.flush()
    ...
    while True:
        _safe_print("[ZCFG]   Page {}...".format(page_num))
        sys.stdout.flush()

        articles, has_more = _crawl_listing_page(page_num)
        if not articles:
            _safe_print("[ZCFG]   No articles on page {}, stopping.".format(page_num))
            sys.stdout.flush()
            break

        all_articles.extend(articles)
        _safe_print("[ZCFG]   -> {} articles (total: {})".format(
            len(articles), len(all_articles)))
        sys.stdout.flush()

        if not has_more:
            _safe_print("[ZCFG]   Last page reached.")
            sys.stdout.flush()
            break
    ...
    _safe_print("[ZCFG] Total articles collected: {}".format(len(all_articles)))
    sys.stdout.flush()
    ...
    if skipped:
        _safe_print("[ZCFG] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[ZCFG] All articles already processed.")
        sys.stdout.flush()
    ...
    _safe_print("\n[ZCFG] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        if elapsed > args.max_runtime - 120:
            _safe_print(
                "\n[ZCFG] Runtime {:.0f}s approaching limit ({}s), "
        ...
        _safe_print("[ZCFG] [{}/{}] {}...".format(idx, len(new_articles), title[:50]))
        sys.stdout.flush()
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[ZCFG]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                _safe_print("[ZCFG]   Uploaded to KB {}".format(args.kb_id))
                sys.stdout.flush()
            except Exception as e:
                logging.error("KB upload failed: %s", e)
                _safe_print("[ZCFG]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            ...
            _safe_print("[ZCFG]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZCFG] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[ZCFG] Stopped early, will resume next run")
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== ZCFG crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "gcjyzx_zcfg_crawler"
