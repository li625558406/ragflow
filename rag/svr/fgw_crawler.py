...
#
"""
Dedicated web crawler for fgw.zhangzhou.gov.cn (漳州市发展和改革委员会).

Crawls multiple content sections (工作动态, 通知公告, 政策文件, 政策解读),
extracts article content from detail pages, converts to Markdown, and uploads
the result to a RAGFlow knowledge base.

Article URL pattern:
  /cms/html/zzsfzhggwyh/YYYY-MM-DD/<digits>.html
...
Usage (typically spawned by task_executor):
    python fgw_crawler.py \
        --tenant-id <TENANT_ID> \
        --target-url http://fgw.zhangzhou.gov.cn/cms/html/zzsfzhggwyh/index.html \
        --kb-id <KB_ID> \
        --task-name <NAME>
"""
...
# ---------------------------------------------------------------------------
# Known sections on fgw.zhangzhou.gov.cn
# ---------------------------------------------------------------------------
SECTIONS = {
    "gzdt": "工作动态",
    "tzgg": "通知公告",
...
def parse_args():
    parser = argparse.ArgumentParser(description="FGW crawler for scheduled tasks")
...
def _extract_articles_from_listing(html, section_label, listing_url=None):
    """Extract article entries from a standard listing page.
    ...
    Also handles the older plain structure:
    ...
    base_for_url = listing_url or f"https://fgw.zhangzhou.gov.cn"
...
def _fetch_article_content(url, client=None):
    """Fetch and parse a fgw.zhangzhou.gov.cn article detail page.
    ...
    Title fallback: <div class="content-title">TITLE</div>
    Content: <div id="Content"><p>...</p></div>
...
def main():
    args = parse_args()
    _safe_print(f"\n{'='*60}")
    _safe_print(f"[FGW] Starting Zhangzhou Development & Reform Commission crawler")
    ...
    _init()
    logging.info("=== FGW crawler started for %s ===", args.target_url)
    ...
    try:
        ...
        for section_path, section_label in selected.items():
            ...
            _safe_print(f"[FGW]   Section '{section_label}': {list_url}")
            sys.stdout.flush()
