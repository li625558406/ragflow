...
#
"""
Dedicated web crawler for Fujian DRC — 解读回应 (fgw.fujian.gov.cn/jdhy/).
...
────────────────────
  ...
  • 4 modules: 新闻发布会, 民生关注, 政策解读, 政策问答
  • 政策解读 (zcjd) has 3 sub-category tabs: bmzcwjjd, szfzcwjjd, qtzcwjjd
  • 政策问答 (zcwd) has 6 sub-categories mixed on one page
  • Listing → /jdhy/<module>/  — <li> items within list_base divs.
  • Detail  → TRS CMS .htm files with <meta name="PubDate">.
...
Usage (typically spawned by task_executor):
    python jdhy_crawler.py \\
        --tenant-id <TENANT_ID> \\
        --target-url http://fgw.fujian.gov.cn/jdhy/ \\
        --kb-id <KB_ID> \\
        --task-name <NAME>
"""
...
def _discover_modules(sess):
    ...
    Returns list of (module_name, listing_url_str) tuples.
    Falls back to _JDHY_MODULES if discovery fails.
    """
    modules = []
    ...
    try:
        ...
        return [(n, urljoin(_SITE_ROOT, p)) for n, p in _JDHY_MODULES]

    # Find module links on the jdhy page
    # These are links like /jdhy/xwfbh/ inside parent elements with class "row"
    for a in soup.find_all('a', href=re.compile(r'/jdhy/[a-z]+/')):
        href = a.get('href', '').strip()
        text = a.get_text(strip=True)
        if not text or len(text) < 2:
            continue

        # Match known module patterns: xwfbh, msgz, zcjd, zcwd
        m = re.match(r'^(?:\.?/jdhy)?/(xwfbh|msgz|zcjd|zcwd)/?$', href)
        if not m:
            continue
...
def _discover_subcategories(sess, module_url):
    ...
    # Find sub-category directories from links on the page.
    # Pattern: /jdhy/<module>/<subcat>/YYYYMM/tYYYYMMDD_NNNNNNN.htm
    # Only match sub-directories UNDER the current module path so we don't
    # pick up cross-links from other modules.
    module_path = urlparse(module_url).path.rstrip('/')

    for a in soup.find_all('a', href=True):
        href = a.get('href', '')
        # Resolve to absolute URL first (handles both relative and absolute)
    ...
    # Prepend main page so it's crawled first, sub-categories after.
...
def _parse_listing(html, listing_url):
    ...
    for li in lis:
        ...
        # Skip external links (e.g., xwfbh links to jyt.fujian.gov.cn)
        if not abs_url.startswith(_SITE_ROOT):
            continue

        # Skip direct attachment links (not detail pages)
...
def _parse_detail(html, detail_url):
    ...
    if title_el:
        # Title may contain breadcrumb-like suffix separated by _
...
def crawl(target_url, output_dir, kb_id, tenant_id, max_runtime=3300):
    ...
    _safe_print("=" * 60)
    _safe_print(f"FGW JDHY Crawler | Today: {today_str}")
...
def main():
    parser = argparse.ArgumentParser(
        description="FGW JDHY Crawler — Fujian DRC 解读回应"
    )
    ...
    parser.add_argument("--task-name", default="jdhy_crawler", help="Task name")
    ...
    init_root_logger("jdhy_crawler")
    logging.info("JDHY Crawler | task=%s | output=%s", args.task_name, output_dir)
