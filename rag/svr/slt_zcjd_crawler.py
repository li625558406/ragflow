...
#
"""
Crawler for slt.fujian.gov.cn — 福建省水利厅 政策解读 (today's data).

Targets:
  1. /xxgk/zcjd/zcjd/bmzcjd/  — 部门政策解读
  2. /xxgk/zcjd/zcjd/szfzcjd/ — 省政府政策解读
  3. /xxgk/zcjd/zcjd/qtzcjd/  — 其他政策解读
...
Data flow
---------
  1. Listing: GET each target URL -> parse .xw-list-1.gl_list > ul > li
     for title, URL, date (YYYY-MM-DD in <span>)
...
# Three target listing pages with their section labels
_LISTING_TARGETS = [
    ("/xxgk/zcjd/zcjd/bmzcjd/",  "部门政策解读"),
    ("/xxgk/zcjd/zcjd/szfzcjd/", "省政府政策解读"),
    ("/xxgk/zcjd/zcjd/qtzcjd/",  "其他政策解读"),
]
...
def _parse_zcjd_listing(html_bytes, base_url, target_date, section_label):
    """Parse a zcjd listing page for articles matching target_date.

    The page contains static-rendered .xw-list-1.gl_list > ul > li blocks.
    Dates are in YYYY-MM-DD format inside <span> elements.
    ...
    for list_div in soup.find_all("div", class_="xw-list-1"):
        ul = list_div.find("ul")
        if not ul:
            continue
...
def _build_markdown(detail, download_dir, source_url, section_label):
...
def parse_args():
    p = argparse.ArgumentParser(
        description="slt.fujian.gov.cn zcjd crawler — 福建省水利厅 政策解读 (today only)"
    )
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[SLT-ZCJD] 福建省水利厅 — 政策解读 crawler")
    _safe_print("[SLT-ZCJD] KB: {}".format(args.kb_id))
    _safe_print("[SLT-ZCJD] Task: {}".format(args.task_name))
    _safe_print("[SLT-ZCJD] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== SLT-ZCJD crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[SLT-ZCJD] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[SLT-ZCJD] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    target_date = _get_target_date()
    _safe_print("[SLT-ZCJD] Target date: {} (8AM rule)".format(target_date))
    sys.stdout.flush()

    crawl_start = time.time()

    # ── Step 1: Crawl all three listing pages ──────────────────────────
    _safe_print("\n[SLT-ZCJD] Step 1/3: Crawling listing pages...")
    sys.stdout.flush()
    ...
    for listing_path, section_label in _LISTING_TARGETS:
        ...
        if remaining < 120:
            _safe_print("[SLT-ZCJD] Runtime limit approaching, skipping remaining sections")
            sys.stdout.flush()
            break

        listing_url = _SITE_ROOT + listing_path
        _safe_print("[SLT-ZCJD]   Fetching: {} ({})".format(listing_url, section_label))
        sys.stdout.flush()

        time.sleep(random.uniform(_REQUEST_DELAY_MIN, _REQUEST_DELAY_MAX))
        html_bytes = _http_get(listing_url, referer=_SITE_ROOT + "/")
        if not html_bytes:
            _safe_print("[SLT-ZCJD]   Failed to fetch, skipping section")
            sys.stdout.flush()
            continue

        articles = _parse_zcjd_listing(html_bytes, listing_url, target_date, section_label)
        _safe_print("[SLT-ZCJD]   -> {} article(s) for {} in {}".format(
            len(articles), target_date, section_label))
        sys.stdout.flush()
        all_articles.extend(articles)

    _safe_print("[SLT-ZCJD]   Total: {} article(s) across {} section(s)".format(
        len(all_articles), len(_LISTING_TARGETS)))
    sys.stdout.flush()

    if not all_articles:
        _safe_print("[SLT-ZCJD] No articles found for today, done.")
        sys.stdout.flush()
        return
    ...
    for sec, cnt in sorted(section_counts.items()):
        _safe_print("[SLT-ZCJD]     {}: {} article(s)".format(sec, cnt))
    sys.stdout.flush()
    ...
    if skipped:
        _safe_print("[SLT-ZCJD] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[SLT-ZCJD] All articles for today already processed.")
        sys.stdout.flush()
        return

    # ── Step 2: Process each article ───────────────────────────────────
    _safe_print("\n[SLT-ZCJD] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        if remaining < 120:
            _safe_print(
                "\n[SLT-ZCJD] Runtime {:.0f}s, {:.0f}s remaining (limit {}s), "
        ...
        if detail_url.lower().endswith(".pdf"):
            ...
            pdf_data = _download_binary(detail_url, referer=_SITE_ROOT + _LISTING_TARGETS[0][0])
            if pdf_data:
                ...
                with open(md_path, "w", encoding="utf-8") as f:
                    f.write(md_content)
                _safe_print("[SLT-ZCJD]   Saved PDF ({} chars)".format(len(md_content)))
                sys.stdout.flush()
                ...
                if args.kb_id:
                    try:
                        ...
                        _safe_print("[SLT-ZCJD]   Uploaded to KB {}".format(args.kb_id))
                        sys.stdout.flush()
                    except Exception as e:
        ...
        time.sleep(random.uniform(_REQUEST_DELAY_MIN, _REQUEST_DELAY_MAX))
        html_bytes = _http_get(detail_url, referer=_SITE_ROOT + _LISTING_TARGETS[0][0])
        if not html_bytes:
        ...
        # Build markdown
        md_content = _build_markdown(detail, article_dl_dir, detail_url, section_label)
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[SLT-ZCJD]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                logging.error("KB upload failed: %s", e)
                _safe_print("[SLT-ZCJD]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            ...
            _safe_print("[SLT-ZCJD]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[SLT-ZCJD] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[SLT-ZCJD] Stopped early, will resume next run")
    _safe_print("[SLT-ZCJD] Target date: {}".format(target_date))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== SLT-ZCJD crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "slt_zcjd_crawler"
