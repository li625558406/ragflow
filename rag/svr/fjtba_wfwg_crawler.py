...
#
"""
Crawler for www.fjtba.com (福建省招标投标协会) — 违法违规公告.

Target: http://www.fjtba.com/PortalPage/ISDInfo.aspx?type=3
Section: 违法违规公告 (type=3), 14 articles across 2 pages.
...
─────────────────────────────────
Listing:
    OPtype=GetArticleList  type=3  pageNo=N  pageSize=10
    Returns ``{"count": 14, "ds": {"ds": [...]}}``

    Each article::
        RN, ID, TITLE, TYPE, TM (ISO date), IMG_AID, CONTENTS (full HTML)

The listing API already returns the full CONTENTS HTML, so we do NOT need
to call the detail API for each article.
...
_LISTING_URL = _SITE_ROOT + "/PortalPage/ISDInfo.aspx?type=3"
_SECTION_TYPE = "3"
_SECTION_LABEL = "违法违规公告"
...
def _crawl_all_listings(seen_ids):
    ...
    if not first_page and total == 0:
        _safe_print("[FJTBA-WFWG] Empty first page, aborting.")
        return all_articles

    total_pages = (total + _PAGE_SIZE - 1) // _PAGE_SIZE if total else 1
    total_pages = min(total_pages, _MAX_PAGES)
    _safe_print("[FJTBA-WFWG] Total: {} article(s), {} page(s)".format(total, total_pages))
    sys.stdout.flush()
    ...
    for page_no in range(2, total_pages + 1):
        ...
        if not articles:
            _safe_print("[FJTBA-WFWG] Page {} returned no articles, stopping.".format(page_no))
            break
...
def parse_args():
    p = argparse.ArgumentParser(
        description="fjtba_wfwg_crawler — 福建省招标投标协会 违法违规公告"
    )
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[FJTBA-WFWG] 福建省招标投标协会 — {} crawler".format(_SECTION_LABEL))
    _safe_print("[FJTBA-WFWG] KB: {}".format(args.kb_id))
    _safe_print("[FJTBA-WFWG] Task: {}".format(args.task_name))
    _safe_print("[FJTBA-WFWG] Max runtime: {}s".format(args.max_runtime))
    _safe_print("[FJTBA-WFWG] Target: {}".format(_LISTING_URL))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== FJTBA-WFWG crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[FJTBA-WFWG] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[FJTBA-WFWG] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    crawl_start = time.time()

    # ── Step 1: Crawl listing via AJAX API ─────────────────────────────
    _safe_print("\n[FJTBA-WFWG] Step 1/3: Crawling listing via AJAX API...")
    sys.stdout.flush()

    seen_ids = set()
    all_articles = _crawl_all_listings(seen_ids)

    _safe_print("[FJTBA-WFWG]   Total: {} article(s) from listing".format(len(all_articles)))
    sys.stdout.flush()

    if not all_articles:
        _safe_print("[FJTBA-WFWG] No articles found, done.")
        sys.stdout.flush()
        return
    ...
    if skipped:
        _safe_print("[FJTBA-WFWG] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[FJTBA-WFWG] All available articles already processed.")
        sys.stdout.flush()
        return

    # ── Step 2: Process each article ───────────────────────────────────
    _safe_print("\n[FJTBA-WFWG] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for i, article in enumerate(new_articles, 1):
        ...
        if remaining < 120:
            _safe_print(
                "\n[FJTBA-WFWG] Runtime {:.0f}s, {:.0f}s remaining (limit {}s), "
        ...
        full_url = "{}/PortalPage/ISDInfoDetail.aspx?type=3&id={}".format(
            _SITE_ROOT, article_id)

        _safe_print("[FJTBA-WFWG] [{}/{}] {}...".format(
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[FJTBA-WFWG]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                _safe_print("[FJTBA-WFWG]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            _save_state(output_dir, {"processed_ids": list(processed_ids)})
            _safe_print("[FJTBA-WFWG]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[FJTBA-WFWG] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[FJTBA-WFWG] Stopped early, will resume next run")
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== FJTBA-WFWG crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "fjtba_wfwg_crawler"
