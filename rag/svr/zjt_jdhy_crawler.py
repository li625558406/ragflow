...
#
"""
Crawler for zjt.fujian.gov.cn — 福建省住房和城乡建设厅 解读回应 (ALL data).

Covers three URL targets:
  1. /jdhy/hygq/     — 回应关切 listing (paginated, ~226 records, 75 static)
  2. /jdhy/zcjd/     — 政策解读 listing (paginated, ~381 records, static pre-rendered)
  3. /jdhy/          — 解读回应 main page (combined display)

Gets ALL list data from static-rendered HTML. Cross-section dedup by article_id.
...
---------
  ...
  2. Detail: GET each .htm detail page -> parse <h1> title, meta tags,
     .TRS_Editor / #detailCont content, .myzj_xl_list attachment links
  3. Attachments: direct download (relative or absolute URLs)
...
# Three listing targets with section labels
_LISTING_TARGETS = [
    ("/jdhy/hygq/", "回应关切"),
    ("/jdhy/zcjd/", "政策解读"),
    ("/jdhy/",      "解读回应"),
...
# Maximum static pages to try (showStatic stepping by 5 for hygq/zcjd)
_MAX_STATIC_PAGES = 30
...
def _parse_hygq_listing(html_bytes, base_url, seen_ids):
    """Parse /jdhy/hygq/ listing page (回应关切).

    Format: .jdhy_list .bd > ul > li > a (with h3 title, no span date).
    Uses showStatic pagination blocks.
    Dates extracted from URL paths.
    """
    if not html_bytes:
        return []
    ...
    # Find all showStatic blocks
    for ul_el in soup.find_all("ul", attrs={"ms-visible": re.compile(r"\$showStatic")}):
        for li in ul_el.find_all("li"):
            ...
            # Non-local URLs (e.g. weixin.qq.com) — skip
            if "http" in href and "zjt.fujian.gov.cn" not in href:
                continue
            ...
            if not date_str:
                # Try to get date from parent path context
                date_str = ""
...
def _parse_zcjd_listing(html_bytes, base_url, seen_ids):
    """Parse /jdhy/zcjd/ listing page (政策解读).

    Format: .gl_list1 ul.changeWj > li with <span>YYYY-MM-DD</span> and <a>.
    Uses showStatic pagination blocks.
    """
    if not html_bytes:
        return []
    ...
    for ul_el in soup.find_all("ul", class_="changeWj"):
        for li in ul_el.find_all("li"):
            a_tag = li.find("a", href=True)
            if not a_tag:
                continue
            ...
            if not date_str:
                date_str = _extract_date_from_url(href) or ""
...
def _parse_jdhy_main(html_bytes, base_url, seen_ids):
    """Parse /jdhy/ main page.

    Sections:
      - .jdhy_list (回应关切) — a > h3 format, no span dates
      - .jdhy_list1 (政策解读) — image carousel
      - .jdhy_list2 (政策解读) — text link list
    """
    if not html_bytes:
        return []
    ...
    # ── .jdhy_list (回应关切) ──
    jdhy_div = soup.find("div", class_="jdhy_list")
    if jdhy_div:
        bd = jdhy_div.find("div", class_="bd")
        if bd:
            ul = bd.find("ul")
            if ul:
    ...
    # ── .jdhy_list1 (政策解读 — image carousel) ──
    list1 = soup.find("div", class_="jdhy_list1")
    if list1:
        for a_tag in list1.find_all("a", href=True):
            href = a_tag["href"].strip()
            title = (a_tag.get("title") or a_tag.get_text()).strip()
            if not title or len(title) < 2:
                continue
    ...
    # ── .jdhy_list2 (政策解读 — text list) ──
    list2 = soup.find("div", class_="jdhy_list2")
    if list2:
        for a_tag in list2.find_all("a", href=True, title=True):
            href = a_tag["href"].strip()
...
def _parse_detail_html(html_bytes, detail_url):
    ...
    # Content: #detailCont .smgb-article or .TRS_Editor
    ...
    # Fallback: standalone .TRS_Editor
    if not info["content_text"]:
        trs_div = soup.find("div", class_="TRS_Editor")
        if trs_div:
            info["content_html"] = trs_div.decode_contents()
            info["content_text"] = trs_div.get_text(separator="\n", strip=True)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="zjt.fujian.gov.cn jdhy crawler — 福建省住房和城乡建设厅 解读回应 (ALL data)"
    )
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZJT-JDHY] 福建省住房和城乡建设厅 — 解读回应 crawler")
    _safe_print("[ZJT-JDHY] KB: {}".format(args.kb_id))
    _safe_print("[ZJT-JDHY] Task: {}".format(args.task_name))
    _safe_print("[ZJT-JDHY] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== ZJT-JDHY crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[ZJT-JDHY] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[ZJT-JDHY] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    crawl_start = time.time()

    # ── Step 1: Crawl all listing pages ────────────────────────────────
    _safe_print("\n[ZJT-JDHY] Step 1/3: Crawling listing pages...")
    sys.stdout.flush()

    seen_ids = set()  # dedup across sections
    all_articles = []

    # 1a. hygq/ sub-page (回应关切)
    for listing_path, section_label in _LISTING_TARGETS:
        elapsed = time.time() - crawl_start
        remaining = args.max_runtime - elapsed
        if remaining < 120:
            _safe_print("[ZJT-JDHY] Runtime limit approaching, skipping remaining sections")
            sys.stdout.flush()
            break

        listing_url = _SITE_ROOT + listing_path
        _safe_print("[ZJT-JDHY]   Fetching: {} ({})".format(listing_url, section_label))
        sys.stdout.flush()

        time.sleep(random.uniform(_REQUEST_DELAY_MIN, _REQUEST_DELAY_MAX))
        html_bytes = _http_get(listing_url, referer=_SITE_ROOT + "/")
        if not html_bytes:
            _safe_print("[ZJT-JDHY]   Failed to fetch, skipping")
            sys.stdout.flush()
            continue

        # Use appropriate parser based on section
        if listing_path == "/jdhy/hygq/":
            articles = _parse_hygq_listing(html_bytes, listing_url, seen_ids)
        elif listing_path == "/jdhy/zcjd/":
            articles = _parse_zcjd_listing(html_bytes, listing_url, seen_ids)
        else:
            articles = _parse_jdhy_main(html_bytes, listing_url, seen_ids)

        _safe_print("[ZJT-JDHY]   -> {} new article(s) in {}".format(
            len(articles), section_label))
        sys.stdout.flush()
        all_articles.extend(articles)

    _safe_print("[ZJT-JDHY]   Total: {} unique article(s) across {} section(s)".format(
        len(all_articles), len(_LISTING_TARGETS)))
    sys.stdout.flush()

    if not all_articles:
        _safe_print("[ZJT-JDHY] No articles found, done.")
        sys.stdout.flush()
        return
    ...
    for sec, cnt in sorted(section_counts.items()):
        _safe_print("[ZJT-JDHY]     {}: {} article(s)".format(sec, cnt))
    sys.stdout.flush()
    ...
    if skipped:
        _safe_print("[ZJT-JDHY] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[ZJT-JDHY] All available articles already processed.")
        sys.stdout.flush()
        return

    # ── Step 2: Process each article ───────────────────────────────────
    _safe_print("\n[ZJT-JDHY] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        if remaining < 120:
            _safe_print(
                "\n[ZJT-JDHY] Runtime {:.0f}s, {:.0f}s remaining (limit {}s), "
        ...
        if detail_url.lower().endswith(".pdf"):
            ...
            pdf_data = _download_binary(detail_url, referer=_SITE_ROOT + "/jdhy/")
            if pdf_data:
                ...
                with open(md_path, "w", encoding="utf-8") as f:
                    f.write(md_content)
                _safe_print("[ZJT-JDHY]   Saved PDF ({} chars)".format(len(md_content)))
                sys.stdout.flush()
                ...
                if args.kb_id:
                    try:
                        ...
                        _safe_print("[ZJT-JDHY]   Uploaded to KB {}".format(args.kb_id))
                        sys.stdout.flush()
                    except Exception as e:
        ...
        time.sleep(random.uniform(_REQUEST_DELAY_MIN, _REQUEST_DELAY_MAX))
        html_bytes = _http_get(detail_url, referer=_SITE_ROOT + "/jdhy/")
        if not html_bytes:
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[ZJT-JDHY]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                logging.error("KB upload failed: %s", e)
                _safe_print("[ZJT-JDHY]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            ...
            _safe_print("[ZJT-JDHY]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZJT-JDHY] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[ZJT-JDHY] Stopped early, will resume next run")
    _safe_print("[ZJT-JDHY] Target: ALL data (static-rendered)")
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== ZJT-JDHY crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "zjt_jdhy_crawler"
