...
#
"""
Dedicated web crawler for ggzyjy.xzfwzx.zhangzhou.gov.cn (漳州市公共资源交易中心).

Crawls multiple content sections (通知公告, 工程建设, 政策法规),
extracts article content from detail pages, converts to Markdown, and uploads
the result to a RAGFlow knowledge base.

This site runs on the YLCMS platform (same as zhangzhou.gov.cn).

Listing page structure:
  <ul class="news-list" id="resources">
    <li><a href="/cms/html/zzggzyjyzx/YYYY-MM-DD/<id>.html" target="_blank">TITLE</a><span>YYYY-MM-DD</span></li>
    ...
...
# ---------------------------------------------------------------------------
# Known sections on ggzyjy.xzfwzx.zhangzhou.gov.cn
# ---------------------------------------------------------------------------
# Key: section identifier
# Value: (URL_path_suffix, display_label)
SECTIONS = {
    "tzgg": ("tzgg/index.html", "通知公告"),
    "gcjs": ("gcjs/index.html", "工程建设"),
    "zcfg": ("zcfg/index.html", "政策法规"),
}

_SITE_ROOT = "http://ggzyjy.xzfwzx.zhangzhou.gov.cn"
_SITE_PREFIX = "/cms/html/zzggzyjyzx"


def parse_args():
    parser = argparse.ArgumentParser(description="GGZYJY crawler for scheduled tasks")
    parser.add_argument("--tenant-id", required=True, help="Tenant ID for KB upload")
    parser.add_argument("--target-url", required=True, help="Homepage URL (e.g. http://ggzyjy.xzfwzx.zhangzhou.gov.cn/)")
...
# ---------------------------------------------------------------------------
# Listing — YLCMS <ul id="resources"> pattern
# ---------------------------------------------------------------------------

def _extract_articles_from_listing(html, section_label, listing_url):
    """Extract article entries from an YLCMS listing page.

    Structure:
      <ul class="news-list" id="resources">
        <li><a href="/cms/html/zzggzyjyzx/YYYY-MM-DD/<id>.html" ...>TITLE</a><span>YYYY-MM-DD</span></li>
        ...

    Some pages have a slideshow carousel before the main <ul id="resources">.
    The actual article list is identified by the presence of <a> tags with
    article URLs.
    ...
    # Find all <ul id="resources"> containers
    for resources_ul in soup.find_all("ul", id="resources"):
        # Check if this list contains article links (not just carousel images)
        article_lis = []
        for li in resources_ul.find_all("li", recursive=True):
            a = li.find("a", href=True)
            if a and a["href"].strip().startswith(_SITE_PREFIX):
                article_lis.append(li)

        if not article_lis:
            continue  # skip carousel / slideshow blocks
    ...
    if not articles:
        logging.warning("No articles found in listing page %s", listing_url)
...
def _fetch_article_content(url):
    """Fetch and parse a ggzyjy article detail page.

    Metadata from <meta> tags:
      <meta name="ArticleTitle" content="...">
      <meta name="PubDate" content="YYYY—MM—DD HH:MM">    (em-dash separator)
    ...
    # Also check for article_attachment div
    ...
    # -- Parse main content from <div class="content" id="Content"> --
...
def main():
    ...
    args = parse_args()
    _safe_print(f"\n{'='*60}")
    _safe_print(f"[GGZYJY] Starting Zhangzhou Public Resources Trading Center crawler")
    _safe_print(f"[GGZYJY] Target URL: {args.target_url}")
    _safe_print(f"[GGZYJY] Task name: {args.task_name}")
    _safe_print(f"[GGZYJY] Target KB: {args.kb_id}")
    _safe_print(f"{'='*60}\n")
    sys.stdout.flush()

    _init()
    logging.info("=== GGZYJY crawler started for %s ===", args.target_url)
    ...
    try:
        ...
        if args.section:
            ...
            if not selected:
                _safe_print(f"[GGZYJY] WARNING: No matching sections found for '{args.section}', using all")
                sys.stdout.flush()
                selected = dict(SECTIONS)
        else:
            selected = dict(SECTIONS)

        _safe_print(f"[GGZYJY] Sections to crawl: {len(selected)}")
        ...
        os.makedirs(output_dir, exist_ok=True)
        _safe_print(f"\n[GGZYJY] Output directory: {output_dir}\n")
        sys.stdout.flush()

        state = _load_state(output_dir) if not args.full else {"processed_urls": []}
        processed_urls = set(state.get("processed_urls", []))
        _safe_print(f"[GGZYJY] Already processed: {len(processed_urls)} URLs\n")
        sys.stdout.flush()

        # 1/5 + 2/5: Crawl listings
        _safe_print(f"[GGZYJY] Step 1/5: Crawling listing pages...")
        sys.stdout.flush()
        ...
        for section_id, (url_path, display_label) in selected.items():
            list_url = f"{_SITE_ROOT}{_SITE_PREFIX}/{url_path}"
            _safe_print(f"[GGZYJY]   Section '{display_label}': {list_url}")
            sys.stdout.flush()
        ...
        _safe_print(f"[GGZYJY] Step 1/5: Collected {len(all_articles)} total articles across {len(selected)} sections\n")
        sys.stdout.flush()

        if not all_articles:
            _safe_print(f"[GGZYJY] No articles found, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"[GGZYJY] Breakdown by section:")
        ...
        if processed_urls:
            ...
            _safe_print(f"\n[GGZYJY] Skipping {len(all_articles) - len(new_articles)} already-processed article(s)")
            sys.stdout.flush()
            all_articles = new_articles

        if not all_articles:
            _safe_print(f"[GGZYJY] All articles already processed, nothing to do")
            sys.stdout.flush()
            sys.exit(0)

        # 3/5 + 4/5: Fetch detail pages
        _safe_print(f"\n[GGZYJY] Step 2/5: Fetching {len(all_articles)} article detail pages...\n")
        sys.stdout.flush()
        ...
        if not md_parts:
            _safe_print(f"[GGZYJY] No articles processed successfully, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"\n[GGZYJY] Detail pages fetched: {success_count} success, {fail_count} failed\n")
        sys.stdout.flush()

        # 5/5: Save + upload
        _safe_print(f"[GGZYJY] Step 3/5: Saving markdown...")
        sys.stdout.flush()
        combined = "\n".join(md_parts)
        filepath = _save_markdown(combined, output_dir)
        _safe_print(f"[GGZYJY] Saved to {filepath} ({len(combined)} chars)\n")
        sys.stdout.flush()
        ...
        _safe_print(f"[GGZYJY] Uploading to KB {args.kb_id}...")
        sys.stdout.flush()
        logging.info("Uploading to KB %s ...", args.kb_id)
        try:
            _upload_to_kb(filepath, args.kb_id, args.tenant_id)
            _safe_print(f"[GGZYJY] Upload complete!\n")
            sys.stdout.flush()
            logging.info("Upload complete")
        except Exception as e:
            _safe_print(f"[GGZYJY] ERROR: Upload failed: {e}")
            sys.stdout.flush()
