...
覆盖页签：
  ...
  3. 规划计划  /cms/html/zzsrmzf/ghjh/index.html
     子栏目: 十四五规划/总体规划/专项规划/计划总结/区域发展规划/国土空间规划/国民经济和社会发展计划报告
  4. 人事信息  /cms/html/zzsrmzf/rsxx/index.html
     子栏目: 奖励表彰 / 人事任免
  5. 人大建议政协提案答复  /cms/html/zzsrmzf/rdjyzxtadf/index.html
     子栏目: 人大代表建议办理复文 / 政协委员提案办理复文
  6. 财政预算  /cms/html/zzsrmzf/czzj/index.html
     子栏目: 财政预决算及三公经费 / 市级预算执行 / 县（区）政府财政预决算报告及三公经费
  7. 行政事业性收费  /cms/html/zzsrmzf/jgysf/index.html
     子栏目: 价费信息 / 民生价格

用法（由 unified_crawler.py 或 trigger 脚本调用）:
    python zhangzhou_zwgk_crawler.py \\
...
# Constants
# ---------------------------------------------------------------------------
_BASE = "https://www.zhangzhou.gov.cn"
_SITE_NAME = "漳州市人民政府"
_SITE_ID = "zhangzhou_zwgk"
_CATEGORY = "news"
_TYPE_NAME = "漳州市人民政府-政务公开"
...
_TABS: List[Dict[str, Any]] = [
    {
        "label": "zfwj",
        "name": "政府文件",
        "url": f"{_BASE}/cms/html/zzsrmzf/zfwj/index.html",
        "sub_sections": [
            {"name": "市政府文件", "url": f"{_BASE}/cms/html/zzsrmzf/szfwj/index.html"},
            {"name": "市政府办文件", "url": f"{_BASE}/cms/html/zzsrmzf/szfbwj/index.html"},
        ],
    },
    {
        "label": "gsgg",
        "name": "公示公告",
        "url": f"{_BASE}/cms/html/zzsrmzf/gsgg/index.html",
        "sub_sections": [],
    },
    {
        "label": "ghjh",
        "name": "规划计划",
        "url": f"{_BASE}/cms/html/zzsrmzf/ghjh/index.html",
        "sub_sections": [
            {"name": "十四五规划", "url": f"{_BASE}/cms/html/zzsrmzf/sswgh/index.html"},
            {"name": "总体规划", "url": f"{_BASE}/cms/html/zzsrmzf/ztgh/index.html"},
            {"name": "专项规划", "url": f"{_BASE}/cms/html/zzsrmzf/zxgh/index.html"},
            {"name": "计划总结", "url": f"{_BASE}/cms/html/zzsrmzf/jhzj/index.html"},
            {"name": "区域发展规划", "url": f"{_BASE}/cms/html/zzsrmzf/qyfzgh/index.html"},
            {"name": "国土空间规划", "url": f"{_BASE}/cms/html/zzsrmzf/gtkjgh/index.html"},
            {"name": "国民经济和社会发展计划报告", "url": f"{_BASE}/cms/html/zzsrmzf/gmjjhshfzjhbg/index.html"},
        ],
    },
    {
        "label": "rsxx",
        "name": "人事信息",
        "url": f"{_BASE}/cms/html/zzsrmzf/rsxx/index.html",
        "sub_sections": [
            {"name": "奖励表彰", "url": f"{_BASE}/cms/html/zzsrmzf/jlbz/index.html"},
            {"name": "人事任免", "url": f"{_BASE}/cms/html/zzsrmzf/rsrm/index.html"},
        ],
    },
    {
        "label": "rdjyzxtadf",
        "name": "人大建议政协提案答复",
        "url": f"{_BASE}/cms/html/zzsrmzf/rdjyzxtadf/index.html",
        "sub_sections": [
            {"name": "人大代表建议办理复文", "url": f"{_BASE}/cms/html/zzsrmzf/rdjy/index.html"},
            {"name": "政协委员提案办理复文", "url": f"{_BASE}/cms/html/zzsrmzf/zxta/index.html"},
        ],
    },
    {
        "label": "czzj",
        "name": "财政预算",
        "url": f"{_BASE}/cms/html/zzsrmzf/czzj/index.html",
        "sub_sections": [
            {"name": "政府财政预决算及三公经费", "url": f"{_BASE}/cms/html/zzsrmzf/czyjsjsgjf/index.html"},
            {"name": "部门财政预决算", "url": f"{_BASE}/cms/html/zzsrmzf/sjyszx/index.html"},
            {"name": "县区财政预决算", "url": f"{_BASE}/cms/html/zzsrmzf/xqzfczyjsbgjsgjf/index.html"},
        ],
    },
    {
        "label": "jgysf",
        "name": "行政事业性收费",
        "url": f"{_BASE}/cms/html/zzsrmzf/jgysf/index.html",
        "sub_sections": [
            {"name": "价费信息", "url": f"{_BASE}/cms/html/zzsrmzf/jfxx/index.html"},
            {"name": "民生价格", "url": f"{_BASE}/cms/html/zzsrmzf/msjg/index.html"},
        ],
    },
]
...
def _extract_site_id(html: str) -> str:
    """从页面 HTML 提取 CMS siteId（用于分页）。"""
...
def _fetch_cms_pages(sess: _requests.Session, url: str, site_id: str,
                     date_filter: str, max_pages: int = 10) -> List[Dict]:
    """通过 CMS 分页获取列表项（page=1 用原始 URL，page>=2 用 sitemanage）。"""
    all_items = []
    seen_urls = set()
    ...
    for page_num in range(1, max_pages + 1):
        if page_num == 1:
            ...
            if not site_id:
                break
            page_url = f"{_BASE}/cms/sitemanage/index.shtml?siteId={site_id}&page={page_num}"
        ...
        try:
            ...
            logging.error("Failed to fetch page %d of %s: %s", page_num, url, e)
            break

        if page_num == 1 and not site_id:
            site_id = _extract_site_id(html)

        items = _extract_list_items_from_html(html, page_url)
        if not items:
            break
        ...
        for item in items:
            ...
            if date_filter:
                if not _item_matches_date(item, date_filter):
                    continue

            page_matched += 1
            all_items.append(item)

        # With date_filter: stop when a page has 0 matching items,
        # since CMS lists are reverse-chronological — all subsequent
        # pages contain even older entries.
        if date_filter and page_matched == 0:
            break
...
def _extract_list_items_from_html(html: str, page_url: str) -> List[Dict]:
    """从 HTML 提取列表项。"""
    soup = BeautifulSoup(html, 'lxml')
    items = []
    seen_urls = set()

    # remove nav/header/footer to avoid false positives
    ...
    for a in soup.find_all('a', href=True):
        ...
        abs_url = _resolve_url(href, page_url)
        if not abs_url:
            continue

        # Filter: only detail page links
        is_detail = (
        ...
        # Exclude index/navigation pages
        if '/index.html' in abs_url or '/index.shtml' in abs_url:
            continue

        # Exclude non-content pages
        skip_patterns = ['/eventdetail/', '/member/', '/jeaf/',
                        '/statistics/', '/lxwm/', '/wzdt/', '/sybz/']
...
def _extract_date_from_element(a_tag, abs_url: str) -> str:
    ...
    if parent:
        ...
        # Match regular dash, em-dash, fullwidth dash
        m = re.search(r'(\d{4})[\-\u2014\uff0d](\d{1,2})[\-\u2014\uff0d](\d{1,2})', parent_text)
        if m:
            return f"{m.group(1)}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"
        # Also try just MM-DD format
        m = re.search(r'(\d{2})[\-\u2014\uff0d](\d{2})', parent_text)
        if m:
            import datetime as _dt
            return f"{_dt.datetime.now().year}-{m.group(1)}-{m.group(2)}"
...
# ---------------------------------------------------------------------------
# Listing: fetch items from each section
# ---------------------------------------------------------------------------
...
def _fetch_list_items(sess: _requests.Session, tab: Dict, date_filter: str) -> List[Dict]:
    ...
    site_id = _extract_site_id(html)
    items = _fetch_cms_pages(sess, url, site_id, date_filter)
    # 分页中的 date_filter 已在 _fetch_cms_pages 内处理

    for it in items:
        it['tab_name'] = tab_name
        it['tab_label'] = tab_label
        it['source_url'] = url

    logging.info("  [%s] %d items from %s (siteId=%s)", tab_name, len(items), url, site_id)
    return items
...
def _fetch_sub_section_items(sess: _requests.Session, sub: Dict,
    ...
    for it in items:
        it['tab_name'] = tab_name
        it['tab_label'] = tab_label
        it['sub_name'] = sub_name
        it['source_url'] = url

    logging.info("    [%s > %s] %d items (siteId=%s)", tab_name, sub_name, len(items), site_id)
    return items
...
def _parse_cms_detail(html: str, detail_url: str) -> Dict:
    ...
    if title_el:
        ...
        # Remove site name suffix: "xxx - 漳州要闻" → "xxx"
        for sep in ('_', ' - ', ' | ', ' — '):
            if sep in text:
    ...
    for meta_name in ('PubDate', 'publishdate', 'articledate', 'dc.date'):
        ...
        if meta and meta.get('content'):
            result['pub_date'] = meta['content'].strip()[:10].replace('\u2014', '-').replace('\uff0d', '-')
            break

    if not result['pub_date']:
        # Unified dash matching (regular dash, em-dash, fullwidth dash)
        m = re.search(r'(\d{4})[\-\u2014\uff0d](\d{1,2})[\-\u2014\uff0d](\d{1,2})', html)
        if m:
            result['pub_date'] = f"{m.group(1)}-{int(m.group(2)):02d}-{int(m.group(3)):02d}"
    ...
    content_selectors = [
        ...
        'div.Custom_UnionStyle', 'div.info-cont', 'div.infoContent',
    ]
        ...
        for tr in trs:
            ...
            if len(cells) >= 2:
                for i, cell in enumerate(cells):
                    if '发布' in cell or '生成' in cell or '公布' in cell:
                        if i + 1 < len(cells):
...
def crawl(tenant_id: str, kb_id: str, task_id: str = "",
          date_filter: str = "", max_runtime: int = 3600):
    """主爬取逻辑。"""
    start_time = time.time()
    date_filter = _normalize_date_filter(date_filter)

    _safe_print("=" * 60)
    _safe_print(f"漳州市人民政府-政务公开 智能采集")
    ...
    # ---- Phase 2: Process each detail page ----
    total_processed = 0
    total_kb = 0
    total_att = 0
    errors = []
    ...
    for idx, item in enumerate(unique_items):
        ...
        if detail.get('attachments'):
            import tempfile
            temp_dir = tempfile.mkdtemp(prefix='zhangzhou_zwgk_')
            for att in detail['attachments']:
        ...
        result_id = _write_to_db(item, detail, _SITE_ID, _CATEGORY, task_id, tenant_id)
        if result_id:
            total_processed += 1

        # ---- Build markdown and upload to KB ----
        if kb_id:
            md_content = _build_markdown(item, detail)

            # Append attachment text to markdown
            ...
            import tempfile
            md_dir = tempfile.mkdtemp(prefix='zhangzhou_zwgk_md_')
...
def run(tenant_id: str = "", kb_id: str = "", task_name: str = "",
    ...
    _safe_print(f"[zhangzhou_zwgk] Custom runner invoked")
...
def main():
    parser = argparse.ArgumentParser(
        description="漳州市人民政府-政务公开 智能采集爬虫"
    )
    parser.add_argument("--tenant-id", required=True, help="Tenant ID")
    parser.add_argument("--kb-id", default=None, help="Knowledge base ID")
    parser.add_argument("--task-name", default="zhangzhou_zwgk", help="Task name")
    ...
    parser.add_argument("--max-runtime", type=int, default=3600, help="Max runtime seconds")
    ...
    init_root_logger("zhangzhou_zwgk")
    ...
    logging.info("Zhangzhou ZWGK Crawler | task=%s | kb=%s | date=%s",
...
if __name__ == "__main__":
    CONSUMER_NAME = "zhangzhou_zwgk_crawler"
