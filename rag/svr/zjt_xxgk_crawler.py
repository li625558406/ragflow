...
#
"""
Crawler for zjt.fujian.gov.cn — 福建省住房和城乡建设厅 政务公开 (today's data).

Covers the /xxgk/ main page which has 4 modules with 10 tabs:
...
_LISTING_URL = "/xxgk/"
_SECTION_LABEL = "福建省住房和城乡建设厅-政务公开"
...
def _parse_xxgk_main(html_bytes, base_url, target_date):
    ...
    The page has 4 role="viewlist" modules. Each module has .tab1.hd with tab
    headers and .bd with content lists (xw-list-1 or zcjd-list format).

    Standard tabs use <span>MM-DD</span> dates. 数据图解 uses URL-embedded dates
    since its listing items lack span elements.
    ...
    # Find all module containers with role="viewlist"
    modules = soup.find_all("div", attrs={"role": "viewlist"})
    if not modules:
        # Fallback: find by .w_item containing .tab1.hd
        modules = []
        for w_item in soup.find_all("div", class_="w_item"):
            if w_item.find("div", class_="tab1"):
                modules.append(w_item)

    for module_div in modules:
        # Get tab headers
        tab_hd = module_div.find("div", class_="tab1")
        if not tab_hd:
            continue

        tab_names = []
        tab_ul = tab_hd.find("ul")
        if tab_ul:
        ...
        # ── Standard xw-list-1 format ──
        xw_lists = bd.find_all("ul", class_="xw-list-1")
        if xw_lists:
            for tab_idx, ul_el in enumerate(xw_lists):
                ...
                for li in ul_el.find_all("li"):
                    ...
                    # Build full date; cross-validate with URL date
        ...
        # ── 数据图解 zcjd-list format (no span dates, extract from URL) ──
        zcjd_lists = bd.find_all("ul", class_=re.compile(r"zcjd-list"))
        if zcjd_lists:
            section_label = tab_names[0] if tab_names else "数据图解"

            for ul_el in zcjd_lists:
                for li in ul_el.find_all("li"):
                    a_tag = li.find("a", href=True)
                    if not a_tag:
                        continue
                    ...
                    # No span date — extract from URL
                    url_date = _extract_date_from_url(href)
                    if not url_date or len(url_date) < 10:
                        continue
...
def parse_args():
    p = argparse.ArgumentParser(
        description="zjt.fujian.gov.cn xxgk crawler — 福建省住房和城乡建设厅 政务公开 (today only)"
    )
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZJT-XXGK] 福建省住房和城乡建设厅 — 政务公开 crawler")
    _safe_print("[ZJT-XXGK] KB: {}".format(args.kb_id))
    _safe_print("[ZJT-XXGK] Task: {}".format(args.task_name))
    _safe_print("[ZJT-XXGK] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== ZJT-XXGK crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[ZJT-XXGK] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[ZJT-XXGK] Already processed: {} article(s)".format(len(processed_ids)))
    sys.stdout.flush()

    target_date = _get_target_date()
    _safe_print("[ZJT-XXGK] Target date: {} (8AM rule)".format(target_date))
    sys.stdout.flush()

    crawl_start = time.time()

    # ── Step 1: Crawl listing page ──────────────────────────────────────
    _safe_print("\n[ZJT-XXGK] Step 1/3: Crawling listing page...")
    sys.stdout.flush()

    listing_url = args.target_url or (_SITE_ROOT + _LISTING_URL)
    _safe_print("[ZJT-XXGK]   Fetching: {}".format(listing_url))
    sys.stdout.flush()

    html_bytes = _http_get(listing_url, referer=_SITE_ROOT + "/")
    if not html_bytes:
        _safe_print("[ZJT-XXGK] Failed to fetch listing page, aborting.")
        sys.stdout.flush()
        return

    all_articles = _parse_xxgk_main(html_bytes, listing_url, target_date)
    _safe_print("[ZJT-XXGK]   -> {} article(s) total for {}".format(len(all_articles), target_date))
    sys.stdout.flush()

    if not all_articles:
        _safe_print("[ZJT-XXGK] No articles found for today, done.")
        sys.stdout.flush()
        return
    ...
    for sec, cnt in sorted(section_counts.items()):
        _safe_print("[ZJT-XXGK]     {}: {} article(s)".format(sec, cnt))
    sys.stdout.flush()
    ...
    if skipped:
        _safe_print("[ZJT-XXGK] {} already processed, {} new".format(skipped, len(new_articles)))
        sys.stdout.flush()

    if not new_articles:
        _safe_print("[ZJT-XXGK] All articles for today already processed.")
        sys.stdout.flush()
        return

    # ── Step 2: Process each article ───────────────────────────────────
    _safe_print("\n[ZJT-XXGK] Step 2/3: Processing {} article(s)...\n".format(len(new_articles)))
    sys.stdout.flush()
    ...
    for idx, article in enumerate(new_articles, 1):
        ...
        if remaining < 120:
            _safe_print(
                "\n[ZJT-XXGK] Runtime {:.0f}s, {:.0f}s remaining (limit {}s), "
        ...
        if detail_url.lower().endswith(".pdf"):
            ...
            if pdf_data:
                ...
                with open(md_path, "w", encoding="utf-8") as f:
                    f.write(md_content)
                _safe_print("[ZJT-XXGK]   Saved PDF ({} chars)".format(len(md_content)))
                sys.stdout.flush()
                ...
                if args.kb_id:
                    try:
                        ...
                        _safe_print("[ZJT-XXGK]   Uploaded to KB {}".format(args.kb_id))
                        sys.stdout.flush()
                    except Exception as e:
        ...
        with open(md_path, "w", encoding="utf-8") as f:
            f.write(md_content)
        _safe_print("[ZJT-XXGK]   Saved ({} chars, {} attachments)".format(
        ...
        if args.kb_id:
            try:
                ...
                logging.error("KB upload failed: %s", e)
                _safe_print("[ZJT-XXGK]   Upload error: {}".format(e))
                sys.stdout.flush()
        ...
        if processed_count % _BATCH_SIZE == 0:
            ...
            _safe_print("[ZJT-XXGK]   Checkpoint ({} processed)".format(processed_count))
            sys.stdout.flush()
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[ZJT-XXGK] Crawl complete — {} new article(s)".format(processed_count))
    if stopped_early:
        _safe_print("[ZJT-XXGK] Stopped early, will resume next run")
    _safe_print("[ZJT-XXGK] Target date: {}".format(target_date))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()
    logging.info("=== ZJT-XXGK crawler finished ===")


if __name__ == "__main__":
    CONSUMER_NAME = "zjt_xxgk_crawler"
