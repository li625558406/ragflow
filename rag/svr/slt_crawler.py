...
#
"""
Dedicated web crawler for slt.fujian.gov.cn (福建省水利厅).

Crawls multiple content sections (水利要闻, 通知公告, 媒体聚焦),
...
Listing page structure:
  <div class="xw-list-1 gl_list" ms-visible="$showStatic(N)">
    <ul>
      <li>
        <i></i>
...
Article URL pattern:
  /wzsy/{section}/{YYYYMM}/t{YYYYMMDD}_{digits}.htm

Content container:
  <div class="xl_con1 smgb-article" id="detailCont">
    <div class="TRS_Editor"><p>...</p></div>
  </div>

Usage (typically spawned by task_executor):
    python slt_crawler.py \
        --tenant-id <TENANT_ID> \
        --target-url https://slt.fujian.gov.cn/ \
        --kb-id <KB_ID> \
        --task-name <NAME>
"""
...
# ---------------------------------------------------------------------------
# Known sections on slt.fujian.gov.cn
# ---------------------------------------------------------------------------
...
def parse_args():
    parser = argparse.ArgumentParser(description="SLT crawler for scheduled tasks")
    parser.add_argument("--tenant-id", required=True, help="Tenant ID for KB upload")
    parser.add_argument("--target-url", required=True, help="Homepage URL (e.g. https://slt.fujian.gov.cn/)")
...
# ---------------------------------------------------------------------------
# Listing — xw-list-1 gl_list with Knockout.js static fallback
# ---------------------------------------------------------------------------

def _extract_articles_from_listing(html, section_label, listing_url):
    """Extract article entries from a slt.fujian.gov.cn listing page.
    ...
    for gl_list in soup.find_all("div", class_="gl_list"):
        for ul in gl_list.find_all("ul", recursive=True):
...
def _fetch_article_content(url, client=None):
    """Fetch and parse a slt.fujian.gov.cn article detail page.
    ...
    Content: <div class="xl_con1 smgb-article" id="detailCont">
              <div class="TRS_Editor"><p>...</p></div>
            </div>
    ...
    resources = []
    seen_srcs = set()
    attach_ul = soup.find("ul", class_="myzj_xl_list")
    if attach_ul:
        for a in attach_ul.find_all("a", href=True):
            href = a["href"].strip()
...
def main():
    args = parse_args()
    _safe_print(f"\n{'='*60}")
    _safe_print(f"[SLT] Starting Fujian Water Resources Department crawler")
    _safe_print(f"[SLT] Target URL: {args.target_url}")
    _safe_print(f"[SLT] Task name: {args.task_name}")
    _safe_print(f"[SLT] Target KB: {args.kb_id}")
    _safe_print(f"{'='*60}\n")
    sys.stdout.flush()

    _init()
    logging.info("=== SLT crawler started for %s ===", args.target_url)
    ...
    try:
        ...
        if args.section:
            ...
            if not selected:
                _safe_print(f"[SLT] WARNING: No matching sections found for '{args.section}', using all")
                sys.stdout.flush()
                selected = dict(SECTIONS)
        else:
            selected = dict(SECTIONS)

        _safe_print(f"[SLT] Sections to crawl: {len(selected)}")
        ...
        os.makedirs(output_dir, exist_ok=True)
        _safe_print(f"\n[SLT] Output directory: {output_dir}\n")
        sys.stdout.flush()

        state = _load_state(output_dir) if not args.full else {"processed_urls": []}
        processed_urls = set(state.get("processed_urls", []))
        _safe_print(f"[SLT] Already processed: {len(processed_urls)} URLs\n")
        sys.stdout.flush()

        # 1/5 + 2/5: Crawl listings
        _safe_print(f"[SLT] Step 1/5: Crawling listing pages...")
        sys.stdout.flush()
        ...
        for section_id, (url_path, display_label) in selected.items():
            ...
            _safe_print(f"[SLT]   Section '{display_label}': {list_url}")
            sys.stdout.flush()
        ...
        _safe_print(f"[SLT] Step 1/5: Collected {len(all_articles)} total articles across {len(selected)} sections\n")
        sys.stdout.flush()

        if not all_articles:
            _safe_print(f"[SLT] No articles found, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"[SLT] Breakdown by section:")
        ...
        if processed_urls:
            ...
            _safe_print(f"\n[SLT] Skipping {len(all_articles) - len(new_articles)} already-processed article(s)")
            sys.stdout.flush()
            all_articles = new_articles

        if not all_articles:
            _safe_print(f"[SLT] All articles already processed, nothing to do")
            sys.stdout.flush()
            sys.exit(0)

        # 3/5 + 4/5: Fetch detail pages
        _safe_print(f"\n[SLT] Step 2/5: Fetching {len(all_articles)} article detail pages...\n")
        sys.stdout.flush()
        ...
        for idx, art in enumerate(all_articles, 1):
            _safe_print(f"[SLT] [{idx}/{total}] {art['section']}: {art['title'][:70]}")
            sys.stdout.flush()
            ...
            if not content:
                _safe_print(f"[SLT]   -> Empty content, skipped")
                sys.stdout.flush()
                fail_count += 1
                continue

            n_files = sum(1 for r in resources if r.get("type") == "file")
            _safe_print(f"[SLT]   -> {len(content)} chars, {n_files} files")
            sys.stdout.flush()
        ...
        if not md_parts:
            _safe_print(f"[SLT] No articles processed successfully, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"\n[SLT] Detail pages fetched: {success_count} success, {fail_count} failed\n")
        sys.stdout.flush()

        # 5/5: Save + upload
        _safe_print(f"[SLT] Step 3/5: Saving markdown...")
        sys.stdout.flush()
        combined = "\n".join(md_parts)
        filepath = _save_markdown(combined, output_dir)
        _safe_print(f"[SLT] Saved to {filepath} ({len(combined)} chars)\n")
        sys.stdout.flush()
        ...
        _safe_print(f"[SLT] Uploading to KB {args.kb_id}...")
        sys.stdout.flush()
        logging.info("Uploading to KB %s ...", args.kb_id)
        try:
            _upload_to_kb(filepath, args.kb_id, args.tenant_id)
            _safe_print(f"[SLT] Upload complete!\n")
            sys.stdout.flush()
            logging.info("Upload complete")
        except Exception as e:
            _safe_print(f"[SLT] ERROR: Upload failed: {e}")
            sys.stdout.flush()
