...
#
"""
Dedicated web crawler for smggzy.sm.gov.cn/smwz/zcfg/ (三明市公共资源交易网 政策法规).

Targets 6 sub-modules under 政策法规:
...
--------------------
  ...
  - Attachments: direct file links (PDF, DOC, XLS, ZIP) from various gov sites
...
--------------------
  ...
  Some modules have only 1 page (totalPageNums undefined = 1).
...
Usage:
    python smzcfg_crawler.py \
        --tenant-id <TENANT_ID> \
        --target-url https://smggzy.sm.gov.cn/smwz/zcfg/ \
        --kb-id <KB_ID> \
        --task-name <NAME>
"""
...
# 6 sub-modules under 政策法规
_MODULES = [
    {"key": "zhfl", "name": "综合类",
     "list_url": "/smwz/zcfg/003001", "parent": "政策法规"},
    {"key": "gcjs", "name": "工程建设",
     "list_url": "/smwz/zcfg/003002", "parent": "政策法规"},
    {"key": "zfcg", "name": "政府采购",
     "list_url": "/smwz/zcfg/003003", "parent": "政策法规"},
    {"key": "tdkc", "name": "土地矿产",
     "list_url": "/smwz/zcfg/003004", "parent": "政策法规"},
    {"key": "cqjy", "name": "产权交易",
     "list_url": "/smwz/zcfg/003005", "parent": "政策法规"},
    {"key": "qt", "name": "其他",
     "list_url": "/smwz/zcfg/003006", "parent": "政策法规"},
]
...
_INFOID_RE = re.compile(
    r"InfoID=([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})",
    re.I,
)
...
def _extract_detail(html, detail_url):
    ...
    # The title appears as free text between breadcrumb and metadata,
    # right before the "信息时间" line
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        # Direct file links (PDF, DOC, XLS, ZIP, RAR, 7Z)
...
def _download_attachment(sess, att, dest_dir):
    """Download an attachment from a direct file URL or bqpoint gateway."""
    ...
    try:
        ...
        if source == "bqpoint":
            # bqpoint is a download gateway page
            resp = sess.get(url, timeout=60)
            if resp.status_code == 200:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="smzcfg crawler - 三明市公共资源交易网 政策法规"
    )
    p.add_argument("--tenant-id", required=True)
    p.add_argument("--target-url",
                   default="https://smggzy.sm.gov.cn/smwz/zcfg/")
    p.add_argument("--kb-id", required=True)
    p.add_argument("--task-name", required=True)
    p.add_argument("--output-dir", default=None)
    p.add_argument("--full", action="store_true")
    p.add_argument("--section", default=None,
                   help="Comma-separated module keys (e.g. zhfl,gcjs)")
...
def main():
    ...
    _safe_print("\n" + "=" * 60)
    _safe_print("[SMZCFG] 三明市公共资源交易网 政策法规 crawler")
    _safe_print("[SMZCFG] KB: {}".format(args.kb_id))
    _safe_print("[SMZCFG] Max runtime: {}s".format(args.max_runtime))
    _safe_print("=" * 60 + "\n")
    sys.stdout.flush()

    settings.init_settings()
    logging.info("=== SMZCFG crawler started ===")

    output_dir = args.output_dir or os.path.join(
        _PROJECT_ROOT, "rag", args.task_name.strip()
    )
    os.makedirs(output_dir, exist_ok=True)
    _safe_print("[SMZCFG] Output: {}\n".format(output_dir))
    sys.stdout.flush()
    ...
    _safe_print("[SMZCFG] Previously processed: {}, completed modules: {}\n".format(
    ...
    _safe_print("[SMZCFG] Active modules: {}".format(
        ", ".join(m["name"] for m in active_modules)))
    sys.stdout.flush()

    sess = _init_session()
    _safe_print("[SMZCFG] Session initialized\n")
    sys.stdout.flush()
    ...
    try:
        ...
        for mod_info in active_modules:
            ...
            if remaining < grace:
                _safe_print(
                    "\n[SMZCFG] Runtime {:.0f}s, remaining {:.0f}s < "
        ...
        _safe_print("\n" + "=" * 60)
        if stopped_early:
            _safe_print("[SMZCFG] Partial run: {} articles. "
                        "Will resume from checkpoint.".format(total_processed))
        else:
            _safe_print("[SMZCFG] Done: {} articles processed.".format(
                total_processed))
        _safe_print("=" * 60 + "\n")
        sys.stdout.flush()
        logging.info("=== SMZCFG crawler finished: %d articles ===",
                     total_processed)
    finally:
        sess.close()
