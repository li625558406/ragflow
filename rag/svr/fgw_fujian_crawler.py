...
#
"""
Dedicated web crawler for fgw.fujian.gov.cn (福建省发展和改革委员会).

Crawls multiple content sections (本委要闻, 市县要闻, 公示公告),
...
Listing page structure (bwdt, sxdt, gsgg):
  <div class="list_base mar_t_small" ms-visible="$showStatic(N)">
    <ul>
      <li>
...
Usage (typically spawned by task_executor):
    python fgw_fujian_crawler.py \
        --tenant-id <TENANT_ID> \
        --target-url http://fgw.fujian.gov.cn/ \
        --kb-id <KB_ID> \
        --task-name <NAME>
"""
...
# ---------------------------------------------------------------------------
# Known sections on fgw.fujian.gov.cn
# ---------------------------------------------------------------------------
# Key: section identifier
# Value: (URL_path_suffix, display_label)
SECTIONS = {
    "bwdt": ("zwgk/xwdt/bwdt/", "本委要闻"),
    "sxdt": ("zwgk/xwdt/sxdt/", "市县要闻"),
    "gsgg": ("zwgk/gsgg/", "公示公告"),
}
...
def parse_args():
    parser = argparse.ArgumentParser(description="FGW Fujian crawler for scheduled tasks")
    parser.add_argument("--tenant-id", required=True, help="Tenant ID for KB upload")
    parser.add_argument("--target-url", required=True, help="Homepage URL (e.g. http://fgw.fujian.gov.cn/)")
...
# ---------------------------------------------------------------------------
# Listing — list_base with Knockout.js static fallback
# ---------------------------------------------------------------------------

def _extract_articles_from_listing(html, section_label, listing_url):
    """Extract article entries from a fgw.fujian.gov.cn listing page.

    Structure (bwdt, sxdt, gsgg):
      <div class="list_base mar_t_small" ms-visible="$showStatic(N)">
        <ul>
          <li>
            <a href="./YYYYMM/t...htm" title="TITLE">TITLE</a>
    ...
    Only articles inside Knockout.js static fallback blocks ($showStatic)
    are collected, which covers ~75 entries per section.
    ...
    # Find static fallback blocks: <div ms-visible="$showStatic(N)">
    for div in soup.find_all("div", attrs={"ms-visible": True}):
        ms_val = div.get("ms-visible", "")
        if not ms_val.startswith("$showStatic"):
            continue
...
def _fetch_article_content(url, client=None):
    """Fetch and parse a fgw.fujian.gov.cn article detail page.
...
def main():
    args = parse_args()
    _safe_print(f"\n{'='*60}")
    _safe_print(f"[FGW] Starting Fujian Development & Reform Commission crawler")
    _safe_print(f"[FGW] Target URL: {args.target_url}")
    _safe_print(f"[FGW] Task name: {args.task_name}")
    _safe_print(f"[FGW] Target KB: {args.kb_id}")
    _safe_print(f"{'='*60}\n")
    sys.stdout.flush()

    _init()
    logging.info("=== FGW Fujian crawler started for %s ===", args.target_url)
    ...
    try:
        ...
        if args.section:
            ...
            if not selected:
                _safe_print(f"[FGW] WARNING: No matching sections found for '{args.section}', using all")
                sys.stdout.flush()
                selected = dict(SECTIONS)
        else:
            selected = dict(SECTIONS)

        _safe_print(f"[FGW] Sections to crawl: {len(selected)}")
        ...
        os.makedirs(output_dir, exist_ok=True)
        _safe_print(f"\n[FGW] Output directory: {output_dir}\n")
        sys.stdout.flush()

        state = _load_state(output_dir) if not args.full else {"processed_urls": []}
        processed_urls = set(state.get("processed_urls", []))
        _safe_print(f"[FGW] Already processed: {len(processed_urls)} URLs\n")
        sys.stdout.flush()

        # 1/5 + 2/5: Crawl listings
        _safe_print(f"[FGW] Step 1/5: Crawling listing pages...")
        sys.stdout.flush()
        ...
        for section_id, (url_path, display_label) in selected.items():
            ...
            _safe_print(f"[FGW]   Section '{display_label}': {list_url}")
            sys.stdout.flush()
        ...
        _safe_print(f"[FGW] Step 1/5: Collected {len(all_articles)} total articles across {len(selected)} sections\n")
        sys.stdout.flush()

        if not all_articles:
            _safe_print(f"[FGW] No articles found, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"[FGW] Breakdown by section:")
        ...
        if processed_urls:
            ...
            _safe_print(f"\n[FGW] Skipping {len(all_articles) - len(new_articles)} already-processed article(s)")
            sys.stdout.flush()
            all_articles = new_articles

        if not all_articles:
            _safe_print(f"[FGW] All articles already processed, nothing to do")
            sys.stdout.flush()
            sys.exit(0)

        # 3/5 + 4/5: Fetch detail pages
        _safe_print(f"\n[FGW] Step 2/5: Fetching {len(all_articles)} article detail pages...\n")
        sys.stdout.flush()
        ...
        for idx, art in enumerate(all_articles, 1):
            _safe_print(f"[FGW] [{idx}/{total}] {art['section']}: {art['title'][:70]}")
            sys.stdout.flush()
            ...
            if not content:
                _safe_print(f"[FGW]   -> Empty content, skipped")
                sys.stdout.flush()
                fail_count += 1
                continue

            n_files = sum(1 for r in resources if r.get("type") == "file")
            _safe_print(f"[FGW]   -> {len(content)} chars, {n_files} files")
            sys.stdout.flush()
        ...
        if not md_parts:
            _safe_print(f"[FGW] No articles processed successfully, exiting")
            sys.stdout.flush()
            sys.exit(0)

        _safe_print(f"\n[FGW] Detail pages fetched: {success_count} success, {fail_count} failed\n")
        sys.stdout.flush()

        # 5/5: Save + upload
        _safe_print(f"[FGW] Step 3/5: Saving markdown...")
        sys.stdout.flush()
        combined = "\n".join(md_parts)
        filepath = _save_markdown(combined, output_dir)
        _safe_print(f"[FGW] Saved to {filepath} ({len(combined)} chars)\n")
        sys.stdout.flush()
        ...
        _safe_print(f"[FGW] Uploading to KB {args.kb_id}...")
        sys.stdout.flush()
        logging.info("Uploading to KB %s ...", args.kb_id)
        try:
            _upload_to_kb(filepath, args.kb_id, args.tenant_id)
            _safe_print(f"[FGW] Upload complete!\n")
            sys.stdout.flush()
            logging.info("Upload complete")
        except Exception as e:
            _safe_print(f"[FGW] ERROR: Upload failed: {e}")
            sys.stdout.flush()
