"""
Trigger fujian_czt_jdhy crawler_task from inside the container,
mirroring crawl4ai_app.trigger_task._run() behavior. Bypasses HTTP/JWT.

Usage (inside container):
    python /ragflow/scripts/_trigger_fujian_czt_jdhy.py
"""
import json
import logging
import subprocess
import sys
import time

sys.path.insert(0, "/ragflow")

from api.db.services.crawler_service import CrawlerTaskService

# 占位：执行前由 _create_fujian_czt_jdhy_task.sql 生成的实际 UUID 替换
TASK_ID = "__FILL_AFTER_INSERT__"

logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s %(message)s")


def main():
    if TASK_ID.startswith("__"):
        # 自动按 site_id 查找最新任务
        from api.db.db_models import CrawlerTask
        rows = list(CrawlerTask.select().where(CrawlerTask.site_id == "fujian_czt_jdhy").order_by(CrawlerTask.create_time.desc()).limit(1))
        if not rows:
            print(f"no crawler_task found for site_id=fujian_czt_jdhy; run _create_fujian_czt_jdhy_task.sql first")
            sys.exit(1)
        task_id_actual = rows[0].id
        print(f"auto-resolved TASK_ID={task_id_actual}")
    else:
        task_id_actual = TASK_ID

    ok, task = CrawlerTaskService.get_by_id(task_id_actual)
    if not ok or not task:
        print(f"task {task_id_actual} not found")
        sys.exit(1)
    print(f"task: name={task.name} site_id={task.site_id} kb_id={task.kb_id} "
          f"last_run_status={task.last_run_status!r}")

    is_first_run = not (task.last_run_status or "").strip()
    script_args_dict = {
        "site_id": task.site_id,
        "writer": "collection",
        "category": "news",
        "task_id": task_id_actual,
    }
    if not is_first_run:
        script_args_dict["date_filter"] = "today"
    script_args = json.dumps(script_args_dict, ensure_ascii=False)
    print(f"is_first_run={is_first_run} script_args={script_args}")

    cmd = [
        "python", "/ragflow/rag/svr/unified_crawler.py",
        "--tenant-id", task.tenant_id,
        "--kb-id", task.kb_id or "",
        "--task-name", f"manual-{task.site_id}",
        "--writer", "collection",
        "--script-args", script_args,
    ]
    proc = subprocess.run(cmd, capture_output=True, text=True, timeout=3600)
    print(f"returncode={proc.returncode}")
    print("---stdout tail---")
    print(proc.stdout[-800:])
    if proc.returncode != 0:
        print("---stderr tail---")
        print(proc.stderr[-1500:])

    status = "success" if proc.returncode == 0 else "fail"
    summary = {
        "status": status,
        "pages": 0,
        "items_found": 0,
        "items_new": 0,
        "kb_uploaded": 0,
        "attachments_uploaded": 0,
        "errors": [] if status == "success" else [(proc.stderr or "fail")[-300:]],
    }
    CrawlerTaskService.update_by_id(task_id_actual, {
        "last_run_status": status,
        "last_run_time": int(time.time() * 1000),
        "last_run_summary": summary,
    })
    print(f"updated last_run_status={status}")


if __name__ == "__main__":
    main()
