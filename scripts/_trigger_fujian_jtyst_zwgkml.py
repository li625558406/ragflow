"""
Trigger fujian_jtyst_zwgkml crawler_task from inside the container,
mirroring crawl4ai_app.trigger_task._run() behavior. Bypasses HTTP/JWT.

用法（容器内）:
  docker cp _trigger_fujian_jtyst_zwgkml.py docker-ragflow-cpu-1:/tmp/
  docker exec docker-ragflow-cpu-1 python /tmp/_trigger_fujian_jtyst_zwgkml.py <TASK_ID>

首次回溯：不带 date_filter（首页 50 条全量）
...
def main():
    if not TASK_ID:
        print("ERROR: 缺少 task_id。用法: python _trigger_fujian_jtyst_zwgkml.py <TASK_ID>")
        sys.exit(2)
    ...
    # category=announcement → collection_writer 走共用主表（无扩展表）
    ...
    # 解析 stdout 末尾 [CRAWLER] Done: N new items 之类的统计
    items_new = 0
