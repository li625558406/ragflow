...
from api.db.db_models import DB, CollectionPolicyExt, CollectionPersonnelExt, CollectionObjectionExt
...
def get_ext_by_category(category: str, result_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    """按 category 路由批量查询扩展字段。非 policy/personnel/objection 返回空 dict。"""
    if not result_ids:
        return {}
    if category == "policy":
...
def upsert_ext_by_category(category: str, result_id: str,
    ...
    """按 category 路由写入扩展字段。非 policy/personnel/objection 直接返回 True（无操作）。"""
    if category == "policy":
