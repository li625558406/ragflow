...
from api.db.db_models import DB, CollectionPolicyExt, CollectionPersonnelExt, CollectionObjectionExt, CollectionZdgksxmlExt
...
class CollectionObjectionExtService(CommonService):
    """collection_objection_ext CRUD。"""
    model = CollectionObjectionExt
...
class CollectionZdgksxmlExtService(CommonService):
    """collection_zdgksxml_ext CRUD（重点公开事项类）。"""
    model = CollectionZdgksxmlExt
    ...
    def upsert(cls, result_id: str, fields: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logging.error("CollectionZdgksxmlExtService.upsert failed (result=%s): %s",
                          result_id, e)
            return False
...
def get_ext_by_category(category: str, result_ids: List[str]) -> Dict[str, Dict[str, Any]]:
    """按 category 路由批量查询扩展字段。非 policy/personnel/objection/zdgksxml 返回空 dict。"""
    if not result_ids:
        return {}
    if category == "policy":
    ...
    if category == "objection":
        return CollectionObjectionExtService.get_by_result_ids(result_ids)
    if category == "zdgksxml":
        return CollectionZdgksxmlExtService.get_by_result_ids(result_ids)
    return {}


def upsert_ext_by_category(category: str, result_id: str,
                           fields: Dict[str, Any]) -> bool:
    """按 category 路由写入扩展字段。非 policy/personnel/objection/zdgksxml 直接返回 True（无操作）。"""
    if category == "policy":
        return CollectionPolicyExtService.upsert(result_id, fields)
    if category == "personnel":
        return CollectionPersonnelExtService.upsert(result_id, fields)
    if category == "objection":
        return CollectionObjectionExtService.upsert(result_id, fields)
    if category == "zdgksxml":
        return CollectionZdgksxmlExtService.upsert(result_id, fields)
    return True
