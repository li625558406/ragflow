...
export function LoopDynamicOutputForm({ node }: IProps) {
  ...
  const childNodeIds = nodes
    .filter(
      (x) =>
        x.parentId === node?.id &&
        x.data.label !== Operator.LoopStart &&
        ...
        return (
          <div key={field.id} className="flex items-center gap-2">
            ...
            <QueryVariable
              ...
              className="w-2/5"
              onChange={(val) => {
                form.setValue(typeField, getType(val));
              }}
              nodeIds={childNodeIds}
            ></QueryVariable>
            <FormField
...
export function LoopDynamicOutput({ node }: IProps) {
  ...
  return (
    <FormContainer>
      ...
      <LoopDynamicOutputForm node={node}></LoopDynamicOutputForm>
    </FormContainer>
  );
}
