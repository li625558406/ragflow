...
| 文档 | 路径 | 包含内容 |
|------|------|----------|
| 迭代记录 | `D:\AI\ragflow2\CHANGE.md` | ★ 全部新增需求/功能整改记录：B端保真预览纯空白 anchor raw 匹配通道+{{key}} 徽标（09-15，最新）/AI 识别 occ 超界溢出封顶（09-14）/PDF 矢量留白横线回填+pdf2docx 引擎替换（09-14 已部署）/文件审核保真预览显原色+显式编辑、「第X章」文本章标题兜底切节、template_fill_events 64KB 截断修复等，最新迭代在最上方 |
