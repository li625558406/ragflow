...
| 文档 | 路径 | 包含内容 |
|------|------|----------|
| 流程页签重写定位范本成稿卡 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25 条目） | ★ 解「范本填写后说『把 6.1法定条件改成XX』定位不到」：法定条件是固定正文非填写点（填写链路含二档降级按设计不碰），DocumentRewrite 才是改正文链路，但 flow-ai-panel 只传 flow_version_id 不传 recent_downloads → 流程页签恒报「没有可用的成稿卡片」。修法=目标优先级「最新产物优先」（成稿卡 > 流程版本）纯前端实现：`collectRecentFillDownloads` 收集 filled download 契约 + 有成稿卡时 `flow_version_id` 置空让位（后端 `_doc_for_action` flow 无条件优先须缺参降级）；document_rewrite.py 仅描述文案。前端 360 passed+build、后端 19 passed；审查 1I+3M（I 已修）（**未部署、未 commit**：部署=前端 build+dist+nginx reload + document_rewrite.py SCP+重启） |
