...
| 范本 binlog 硬删恢复方法论 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（八）条目） | ★ 范本误删救援+写回回滚+字节/元数据错配重识别全套运维实录（6efdafb4 恢复、193 点筛选集回退 v7、适配新范本 b5b98da0 v2）：binlog ROW+FULL 行镜像重插（mysqlbinlog 在宿主机、字面量反转义、14/11 列列序）、MinIO versioning Off 不可反删、published 范本 save_placeholders 自动升版（重识别 454 落 v6、筛选集 193 落 v7）、`_merge_defaults` 显式 default_value 优先保跨范本迁移零丢失；193 点存档 `.scratch/tpl6efd_193_placeholders.json` |
| 范本删除守卫（进行中流程拒删） | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（七）条目） | ★ 解「删 B 端范本要判断是否正被流程使用」：delete_template 级联清理会连带删流程依赖的填写任务行（删后 filled 行轮询恒 102）。修法=`_active_flow_usage` 关联链路——`tpl_fill_task.flow_instance_id` 存**画布会话 id**（非 flow_id）→ `FlowAiChat.session_id` 反查 → `FlowInstance`；`deleted=0 且 status!=cancelled` 任一存在即拒删并提示流程名（归档视为在用；已删/作废不阻塞；旧数据空串不阻塞）。后端 2 文件 110 passed（**已部署 2026-09-25 + push c6ad4ef2**：单文件 md5 一致+重启+import 冒烟） |
| 写回范本库二次确认 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（六）条目） | ★ C端流程页「写回范本库」按钮加 AlertDialog 二次确认（写回会把本轮字段值沉淀为范本默认值、影响后续所有轮次兜底值，误触代价高）；按钮在共享组件 TemplateFillSedimentButton，c-chat 对话页+流程页签两入口同时生效；错误态重试同样先过确认。纯前端 2 文件 12 用例（**已部署 2026-09-25 + push 408ea327**：chunk md5 双端一致） |
| 终态预览渲染源切成稿 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（五）条目） | ★ 解「（四）桥接后点开预览还是改前内容」：**第四层断层（最后一层）**——「查看填写内容」预览渲染源是**模板工作副本**（含 {{key}}），replace/rewrite 改的非填写点正文只存在于成稿，工作副本永不变，预览刷新/重开都没用。修法=终态+download 契约时渲染源切 `tplfill-{task_id}` 派生副本（新 `useTemplateFillResultFile` fetch 带 Authorization），失败回落工作副本；成稿视图占位符高亮/定位自然失效（可接受）。纯前端 4 文件 335 passed（**已部署 2026-09-25 + push e5684a5c**：chunk md5 双端一致） |
| DocumentRewrite 桥接 tplfill 派生副本 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（四）条目） | ★ 解「流程 demo03 说改成 LG11111 成功但文档里看不到」：replace 实际成功（v2 字节含 LG11111），但填写进度卡预览/下载走 `tplfill-{task_id}` 派生副本未跟随更新——与 09-17 modify 同类缺口，replace/rewrite 漏了桥接。修法=`_bridge_fill_copy` 同名覆盖派生副本（失败降级为回执提示）+ `_doc_for_action` 扩 7 元组带 doc_id。59 passed（**已部署 2026-09-25 + push 259b0c9d**：单文件 md5 一致+重启+冒烟通过；demo03 派生副本已顺手覆盖回读验证） |
| DocumentRewrite outline 取原文+参数颠倒防御 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（三）条目） | ★ 解「（二）上线后流程【LG】测试02 仍替换失败且第一遍没识别到」：replace 机制正常（v3 段数 977 无丢失），败在 agent 填参——outline 只有章节标题拿不到原文，第一遍**参数填反**（find='6.1法定条件' 短语、replace=整句）致重复膨胀污染段，第二遍替换残留尾巴。修法=outline 新增可选 keyword 返回含关键词段落**完整原文**（replace 前取逐字原文，禁止猜）+ replace 方向闸（find_text 是 replace_text 子串即拒绝）。后端单文件 57 passed（**已部署 2026-09-25 + push 4a465f02**：单文件 md5 双端一致+重启+冒烟 keyword 声明通过，前端零改动；部署后需对 v3 污染段再发一次精准替换修正） |
| DocumentRewrite 精准替换+截断根修 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25（二）条目） | ★ 解「说改成 LG11111，AI 说已改好但文档没变」：生产实锤三层缺陷——①切节只到章级致 agent 猜错节（6.1 在第 4 节却重写了第 5 节）；②单句替换被设计成整章 LLM 全量重写；③`truncated 92 -> 50` 静默截断丢 49 段仍照常落版本。修法=新增 action=replace（find_text/replace_text docx 文本层逐字替换，零 LLM 重生成零损伤，找不到零改动给引导）+ rewriter 超限改拒绝触发重试 + 输出体量闸（<源文 60% 拒绝保存防丢文）。后端 3 文件 88 passed（**已部署 2026-09-25 + push 323381d4**：3 文件 md5 双端一致+重启+冒烟 replace 枚举/ratio=0.6 通过，前端零改动） |
| 流程页签重写定位范本成稿卡 | `D:\AI\ragflow2\CHANGE.md`（2026-09-25 条目） | ★ 解「范本填写后说『把 6.1法定条件改成XX』定位不到」：法定条件是固定正文非填写点（填写链路含二档降级按设计不碰），DocumentRewrite 才是改正文链路，但 flow-ai-panel 只传 flow_version_id 不传 recent_downloads → 流程页签恒报「没有可用的成稿卡片」。修法=目标优先级「最新产物优先」（成稿卡 > 流程版本）纯前端实现：`collectRecentFillDownloads` 收集 filled download 契约 + 有成稿卡时 `flow_version_id` 置空让位（后端 `_doc_for_action` flow 无条件优先须缺参降级）；document_rewrite.py 仅描述文案。前端 360 passed+build、后端 19 passed；审查 1I+3M（I 已修）（**已部署 2026-09-25**：前端 dist 上传+nginx reload，后端 document_rewrite.py SCP md5 一致+容器重启+冒烟通过） |
