...
## 2026-09-25（十二）终态预览默认展示成稿（用户拍板，第三次迭代）（已部署 2026-09-25）

**主题**：用户报「之前改的，这个替换又不行了？」——服务器字节级验证**替换链路完全正常**（rewrite v2 落库、派生副本 `tplfill-{task_id}` 已桥接覆盖、剥 XML 标签后 `6.1法定条件：LG11111` 存在；第一轮 bytes 搜索 False 是 docx 拆 run 假阴性）。真正原因=（九）把终态默认改成填写视图，replace 改的固定正文只在成稿。用户拍板「那就默认展示成稿效果」→ 渲染源默认第三次翻转。

**改动**（纯前端 2 文件，5 套件 56 passed）：`template-fill-live-preview.tsx` `preferResult` 初始值 false→true（`showResult = wantResult && preferResult && !resultError` 结构不动）——终态+下载契约默认渲染成稿派生副本，「返回填写视图」切回工作副本看占位符徽标/点击定位；范本切换重置 true；成稿拉取失败回落工作副本不变。测试：切换 describe 断言反转（默认成稿/返回切填写/再切成稿），套件头注释同步三迭代史。

**视图分工固有代价**（两视图互斥，终态用户自选）：成稿视图=正文修改可见但占位符失效；填写视图=占位符徽标/定位可用但无正文修改。

**部署**：build + dist 上传 + nginx reload，全量 md5 双端一致（f27ce165），首页 200。
...
**改动**（后端单文件 `agent/tools/document_rewrite.py` + 测试 +4 用例，doc_rewrite 三套件 57 passed）：
- `outline` 新增可选 `keyword`：返回含关键词的段落**完整原文**（含标点，每段截 2000 字、最多 10 段、零命中给引导），供 replace 取逐字原文——第一性原理：精准替换的前提是拿到精确原文，禁止凭记忆猜。
- `replace` 新增**方向闸**：`find_text in replace_text` 即拒绝（参数颠倒的特征签名——旧短语当 find、长新文当 replace 会让文档越替换越膨胀），零改动不落版本。
- meta description 强化了标准流程（outline(keyword) 取原文 → replace）与方向不能反；parameters 新增 keyword 声明。
- 测试：outline keyword 命中返回原文/零命中引导/参数颠倒拒绝（不落版本）/meta 含 keyword。

**遗留**：①【LG】测试02 的 v3 段 40 仍是污染状态，修复部署后需再发一次精准替换修正（find_text=完整污染句 → replace_text=`6.1法定条件：LG11111`）；②（二）遗留各项继续有效。

**部署清单**（后端单文件 SCP + 容器重启）：`agent/tools/document_rewrite.py`；前端零改动。

## 2026-09-25（二）DocumentRewrite 精准替换 replace + 截断数据丢失根修（已部署 2026-09-25 + push 323381d4）
...
**改动（纯前端 2 文件）**：
- `global.less` 新增作用域类 `.flow-chat-md`：表格（边框 `#e5e6eb`+表头灰底 `#f5f7fa` nowrap+13px 字号+`display:block; overflow-x:auto` 宽表横向滚动不撑破气泡）、标题 h1~h6 收敛 14~16px（治默认 h1/h2 巨字号）、列表/段落/引用/分割线间距统一、代码块/行内代码底色圆角、链接 `#1a66fb`、气泡内首尾元素去边距。
- `flow-detail.tsx` ConversationView 两处回复气泡（历史记录+live 流式）挂 `flow-chat-md` 类。

**关键决策**：样式挂在 `global.less` 作用域类而非改共享组件（`next-markdown-content`/`chaptered-markdown`），c-chat 等其它使用方零影响；**存量数据天然生效**（渲染是展示时行为，不改库数据），生产实证该条存量消息部署后即为带边框表格。

**验证**：全量 vitest **29 套件 352 passed**；less 编译校验通过；部署后 index.html 与 CSS 产物 `index-BjR4GuwA.css`（含 `flow-chat-md`）双端 md5 一致、首页/CSS 均 200。
